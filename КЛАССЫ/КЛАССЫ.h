...
protected:
    ...
    Otrezok* Next;
public:
    Otrezok(pair<double, double>* _start, pair<double, double>* _end, double _N)
    {
        start = _start;
        end = _end;
    ...
    void ChangeCharacteristic(double _m, double _N);
    double GetCharacteristic();
    void SetEnd(pair<double, double>* _end, double _N);
...
public:
    Otrezki(Otrezok* _Head, double _r, double _N)
    {
        Head = _Head;
        r = _r;
        if (Head->GetM() != 0)
    ...
    void Add(pair<double, double>* _tmp, double _N);
    double Getm();
...
public:
	CenterSquare(double _x1, double _x2, List _Type, CenterSquare* _Next, CenterSquare* _Previous, double i, double h1, double h2)
	{
		x1x2.first = _x1;
		x1x2.second = _x2;
		Type = _Type;
	...
	void SetStart(CenterSquare* _Start);
	void SetEnd(CenterSquare* _End);
...
protected:
	...
	CenterSquare* End;
public:
	Hilbert_Curve_2D(double a, double b, double c, double d, double m)
	{
		Head = new CenterSquare((b - a) / 2.0, (d - c) / 2.0, Top, nullptr, nullptr, 1, (b - a) / 4.0, (d - c) / 4.0);
		End = Head->GetEnd();
		Head = Head->GetStart();
		m--;
		double h1 = (b - a) / 4.0;
