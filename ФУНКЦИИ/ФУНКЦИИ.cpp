...
extern "C" __declspec(dllexport) double Characteristic(double m, double x1, double x2, double y1, double y2, double _N)
{
    double delta_x = pow(abs(x2 - x1), (1 / _N));
    return ((delta_x) + pow((y2 - y1), 2) / (pow(m, 2) * (delta_x)) - 2 * (y2 + y1) / m);
}
extern "C" __declspec(dllexport) double Shag(double m, double x1, double x2, double y1, double y2, double _N)
{
    return ((x1 + x2) / 2) - Sign(y2 - y1) * pow((abs(y2 - y1) / (2 * m)), _N);
}